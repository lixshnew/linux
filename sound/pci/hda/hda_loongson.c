...
#define ICH_NUM_CAPTURE  4
#define ICH_NUM_PLAYBACK 4
...
MODULE_LICENSE("GPL");
MODULE_SUPPORTED_DEVICE("{Loongson, LS7A}");
...
static char *driver_short_names[] = {
	...
	[AZX_DRIVER_LS2K] = "HD-Audio Loongson",
		      ...
		      unsigned int driver_caps, struct azx **rchip)
{
	...
	*rchip = NULL;
	...
	if(model[dev])
		...
		of_property_read_string(platdev->dev.of_node, "modelname", &modelname);

	err = azx_bus_init(chip, modelname);
	if (err < 0) {
		kfree(hda);
		return err;
	}
...
static int azx_first_init(struct azx *chip)
{
	...
	if (!chip->playback_streams && !chip->capture_streams) {
		...
		chip->capture_streams = ICH_NUM_CAPTURE;
		chip->playback_streams = ICH_NUM_PLAYBACK;
	}
...
static int azx_plat_probe(struct platform_device *pdev)
{
	int err;
	bool probe_now;
	static int dev;
	...
	err = azx_create(card, NULL, pdev, dev, AZX_DRIVER_ICH | AZX_DCAPS_LS2X_WORKAROUND, &chip);
	if (err < 0)
		goto out_free;
...
static int azx_plat_remove(struct platform_device *pdev)
{
	return snd_card_free(dev_get_drvdata(&pdev->dev));
}

static void azx_plat_shutdown(struct platform_device *pdev)
{
...
#ifdef CONFIG_OF
static struct of_device_id audio_id_table[] = {
	{ .compatible = "loongson,ls2h-audio", },
	{ .compatible = "loongson,ls2k-audio", },
	{ /* end */ },
};
MODULE_DEVICE_TABLE(of, audio_id_table);
#endif

/* platform_driver definition */
static struct platform_driver azx_plat_driver = {
	.probe = azx_plat_probe,
	.remove = azx_plat_remove,
	.shutdown = azx_plat_shutdown,
	.driver = {
		.name = "loongson-audio",
		.owner = THIS_MODULE,
		.pm = AZX_PM_OPS,
#ifdef CONFIG_OF
		.of_match_table = of_match_ptr(audio_id_table),
#endif
	},
};
...
static int azx_pci_probe(struct pci_dev *pdev, const struct pci_device_id *pid)
{
	...
	ret = azx_create(card, pdev, NULL, dev, AZX_DRIVER_LS7A | AZX_DCAPS_LS2X_WORKAROUND, &chip);
	if (ret < 0)
		goto out_free;
	card->private_data = chip;
	hda = container_of(chip, struct hda_loongson, chip);
...
static int __init alsa_card_azx_init(void)
{
	...
	return platform_driver_register(&azx_plat_driver);
}

static void __exit alsa_card_azx_exit(void)
{
	pci_unregister_driver(&azx_pci_driver);
	platform_driver_unregister(&azx_plat_driver);
}
