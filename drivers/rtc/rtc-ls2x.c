...
static int ls2x_rtc_probe(struct platform_device *pdev)
{
	...
	if (!res) {
		pr_err("%s: RTC resource data missing\n", pdev->name);
		return -ENOENT;
	}

	mem = request_mem_region(res->start, resource_size(res), pdev->name);
	if (!mem) {
		pr_err("%s: RTC registers at %x are not free\n",
	...
	if (!info->rtc_base) {
		pr_err("%s: RTC registers can't be mapped\n", pdev->name);
		goto fail1;
	}
	...
	if (IS_ERR(info->rtc_dev)) {
		pr_err("%s: can't register RTC device, err %ld\n",
	...
	check_and_init_vaild_rtc_time(rtc);
