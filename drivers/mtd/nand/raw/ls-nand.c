...
#include <linux/platform_data/ls-nand.h>
...
static int ls_nand_ecc_calculate(struct nand_chip *chip,
				const uint8_t *dat, uint8_t *ecc_code)
{
	return 0;
}

static int ls_nand_ecc_correct(struct nand_chip *chip,
...
static void ls_nand_ecc_hwctl(struct nand_chip *chip, int mode)
{
...
static int ls_nand_waitfunc(struct nand_chip *chip)
{
...
static void ls_nand_select_chip(struct nand_chip *chip, int chipnr)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct ls_nand_info *info = mtd->priv;

	info->cs = (chipnr == -1) ? info->cs0 : info->cs0 + chipnr;
}

static int ls_nand_dev_ready(struct nand_chip *chip)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct ls_nand_info *info = mtd->priv;
...
static int get_chip_capa_num(struct nand_chip *chip)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	uint64_t chipsize = nanddev_target_size(&chip->base);
...
static int ls_nand_attch_chip(struct nand_chip *chip)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct ls_nand_info *info = mtd->priv;
	int exit_nand_size;

	exit_nand_size = get_chip_capa_num(chip);

	nand_writel(info, NAND_PARAM_REG, (nand_readl(info, NAND_PARAM_REG) & 0xfffff0ff) | (exit_nand_size << 8));
	chip->legacy.cmdfunc(chip, NAND_CMD_RESET, 0, 0);
	return 0;
}

static const struct nand_controller_ops ls_nand_controller_ops = {
	.attach_chip = ls_nand_attch_chip,
};
...
static void ls_read_id(struct ls_nand_info *info)
{
	...
	if (mtd->writesize) {
		chip_cap = get_chip_capa_num(&info->nand_chip);
...
static void ls_nand_cmdfunc(struct nand_chip *chip, unsigned int command,
...
static uint8_t ls_nand_read_byte(struct nand_chip *chip)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct ls_nand_info *info = mtd->priv;
	unsigned long flags;
...
static void ls_nand_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct ls_nand_info *info = mtd->priv;
...
static void ls_nand_write_buf(struct nand_chip *chip, const uint8_t *buf,
				int len)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct ls_nand_info *info = mtd->priv;
			       ...
			       struct ls_nand_info *info)
{
	...
	this->controller->ops = &ls_nand_controller_ops;

	// this->options		= 8;
	this->options		= NAND_CACHEPRG;
	this->legacy.waitfunc		= ls_nand_waitfunc;
	this->legacy.select_chip	= ls_nand_select_chip;
	this->legacy.dev_ready		= ls_nand_dev_ready;
	this->legacy.cmdfunc		= ls_nand_cmdfunc;

	// this->legacy.read_word		= ls_nand_read_word;
	this->legacy.read_byte		= ls_nand_read_byte;
	this->legacy.read_buf		= ls_nand_read_buf;
	this->legacy.write_buf		= ls_nand_write_buf;
...
static int ls_nand_probe(struct platform_device *pdev)
{
	...
	u32 data;
	// u64 data64;
	resource_size_t dma_base_reg_addr;
	...
	if (data == 2) {
		...
		if (of_property != 0)
			dma_base_reg_addr = (resource_size_t)of_read_number(of_property, 2);
	} else {
		of_property_read_u32(chan->device->dev->of_node, "reg", &data);
		dma_base_reg_addr = (resource_size_t)data;
	}

	info->dma_order_reg = (u64)ioremap(dma_base_reg_addr, 8);
	...
	if (of_property_read_string(pdev->dev.of_node, "nand-dev-name", &mtd->name))
