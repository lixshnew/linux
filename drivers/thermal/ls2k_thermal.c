...
DEFINE_SPINLOCK(ls2k_tsensor_lock);
...
static int ls2k_thermal_get_temp(void *__data, int *temp)
{
	...
	*temp = ((reg_val & 0xff) - THSENS_OUT_OFFSET) * 1000;

	return 0;
}

static irqreturn_t ls2k_thermal_irq_handler(int irq, void *dev_id)
{
	struct ls2k_thermal_data *data = dev_id;
...
static int ls2k_thermal_probe(struct platform_device *pdev)
{
	...
	data->irq_handler = ls2k_thermal_irq_handler;
	...
	ret = devm_request_threaded_irq(dev, data->irq, NULL, ls2k_thermal_irq_handler,
			IRQF_ONESHOT | IRQF_SHARED, "ls2k_thermal", data);
	if (ret < 0) {
