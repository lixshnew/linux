...
static int eiointc_router_init(unsigned int cpu)
{
	...
	if ((cpu_logical_map(cpu) % CORES_PER_EIO_NODE) == 0) {
		...
		if (!eiointc_priv[0]->misc_func) {
			eiointc_enable();
		} else {
			val = iocsr_readq(eiointc_priv[0]->misc_func);
			val |= (1 << eiointc_priv[0]->eio_en_off);
			iocsr_writeq(val, eiointc_priv[0]->misc_func);
		}
		...
		for (i = 0; i < VEC_COUNT / 32; i++) {
			iocsr_writel(0, EIOINTC_REG_ENABLE + i * 4);
			iocsr_writel(0xffffffff, EIOINTC_REG_BOUNCE + i * 4);
		}
	}
...
static void eiointc_mask_irq(struct irq_data *d)
{
	...
	val = iocsr_readl(EIOINTC_REG_ENABLE + (hwirq >> 5) * 4);
	val &= ~(1 << hwirq);
	iocsr_writel(val, EIOINTC_REG_ENABLE + (hwirq >> 5) * 4);
}
...
static struct irq_chip eiointc_irq_chip = {
	...
	.irq_eoi		= eiointc_unmask_irq,
	.irq_enable		= eiointc_unmask_irq,
	.irq_disable		= eiointc_mask_irq,
...
int extioi_vec_init(struct fwnode_handle *fwnode, int cascade,
		...
		u32 misc_func,
		u32 eio_en_off,
		bool is_default_domain,
		u64 node_map,
		u32 node)
{
	int err, i;

	eiointc_priv[nr_pics] = kzalloc(sizeof(struct eiointc_priv), GFP_KERNEL);
	if (!eiointc_priv[nr_pics])
		return -ENOMEM;

	eiointc_priv[nr_pics]->eiointc_domain = irq_domain_create_linear(fwnode, vec_count,
					&eiointc_domain_ops, eiointc_priv[nr_pics]);
	if (!eiointc_priv[nr_pics]->eiointc_domain) {
		pr_err("loongson-eiointc: cannot add IRQ domain\n");
		err = -ENOMEM;
		goto failed_exit;
	}
	eiointc_priv[nr_pics]->misc_func = misc_func;
	eiointc_priv[nr_pics]->eio_en_off = eio_en_off;
	eiointc_priv[nr_pics]->vec_count = vec_count;

	node_map = node_map ? node_map : -1ULL;
	for_each_possible_cpu(i) {
		if (node_map & (1ULL << (cpu_to_eio_node(i)))) {
			node_set(cpu_to_eio_node(i), eiointc_priv[nr_pics]->node_map);
			cpumask_or(&eiointc_priv[nr_pics]->cpuspan_map,
				&eiointc_priv[nr_pics]->cpuspan_map, cpumask_of(i));
		}
	}
	eiointc_priv[nr_pics]->node = node;

	irq_set_chained_handler_and_data(cascade,
					eiointc_irq_dispatch,
					eiointc_priv[nr_pics]);
	eiointc_priv[nr_pics]->domain_handle = fwnode;

	if (is_default_domain)
		irq_set_default_host(eiointc_priv[nr_pics]->eiointc_domain);
				...
				struct device_node *parent)
{
	...
	is_default_domain = of_property_read_bool(of_node, "default-irq-domain");
