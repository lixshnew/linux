...
#define GPIO_INT(x)	                (x->base + x->int_offset)
...
static int loongson_gpio_to_irq(struct gpio_chip *chip, unsigned int offset)
{
	...
	if (lgpio->support_irq) {
		/* 2k1000或2k500的gpio中断有些是分组共享的，但每一组共享中断中只能有一个gpio用于中断，
		如果需要使用多于一个gpio用于中断，硬件设计时要注意，把用于中断的gpio分到不同的组，
		如何分组，请参考数据手册。
		注意2k500的gpio分组中断，分组中最高位的gpio是不能用于中断的 */
		u64 temp = readq(GPIO_INT(lgpio));
		writeq(temp | (0x1ull << offset), GPIO_INT(lgpio));
	}
			    ...
			    void __iomem *base)
{
	...
	if (!strcmp(lgpio->chip.label, "loongson,ls7a-gpio") ||
		...
		if (lgpio->support_irq) {
			...
			writeq(0x00, GPIO_INT(lgpio));
		} else {
			lgpio->chip.to_irq = NULL;
		}
	}
				  ...
				  struct loongson_gpio_chip *lgpio)
{
	...
	of_property_read_u32(np, "int_offset", (u32 *)&lgpio->int_offset);
				  ...
				  struct loongson_gpio_chip *lgpio)
{
	...
	device_property_read_u32(dev, "int_offset", (u32 *)&lgpio->int_offset);
