...
static int arch_pci_probe_root_resources(struct acpi_pci_root_info *info)
{
	...
	struct resource_entry isa_entry, *entry, *tmp;
	unsigned long flags;
	struct resource *res;
	...
	if (ret < 0)
		...
		/* register legacy isa io, used by EC */
		memset(&isa_entry, 0, sizeof(isa_entry));
		isa_entry.res = &isa_entry.__res;
		isa_entry.res->start = LOONGSON_LIO_BASE;
		isa_entry.res->end = LOONGSON_LIO_BASE + ISA_IOSIZE - 1;
		isa_entry.res->flags = IORESOURCE_IO;
		isa_entry.offset = LOONGSON_LIO_BASE;

		fwnode = kzalloc(sizeof(*fwnode), GFP_ATOMIC);
		arch_pci_root_remap_iospace(fwnode, &isa_entry);
