...
int param_set_dcount(const char *val, const struct kernel_param *kp)
{
	...
	if (ret < 0)
		return ret;
	dbcn = csr_readl(LOONGARCH_CSR_MWPC) & 0x3f;
	if (d > dbcn)
		return -EINVAL;
...
int param_set_icount(const char *val, const struct kernel_param *kp)
{
	...
	if (ret < 0)
		return ret;
	ibcn = csr_readl(LOONGARCH_CSR_FWPC) & 0x3f;
	if (d > ibcn)
		return -EINVAL;
...
{
	csr_xchgl(0, CSR_CRMD_WE, LOONGARCH_CSR_CRMD);
	regs->csr_prmd &= ~CSR_PRMD_PWE;
}
...
static void kgdb_correct_hw_break(void)
{
	...
	csr_xchgl(activated ? CSR_CRMD_WE : 0, CSR_CRMD_WE, LOONGARCH_CSR_CRMD);
