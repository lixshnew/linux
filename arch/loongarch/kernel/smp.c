...
{
	unsigned int cpu;
	struct thread_info* cur_thread_info;
	...
	cur_thread_info = current_thread_info();
	if (cur_thread_info->preempt_count != INIT_PREEMPT_COUNT)
		cur_thread_info->preempt_count = INIT_PREEMPT_COUNT;
